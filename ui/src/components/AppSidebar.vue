  ...
  <div :style="s.sidebar">
    ...
    <div :style="s.projWrap">
      ...
      <button v-if="projects.length === 0" :style="s.addProjectBtn" @click="$emit('add-project')">
        <AppIcon name="folder-git-2" :size="13" :extra-style="{ color: 'var(--color-gray-500)', flexShrink: 0 }" />
        <span :style="{ flex: 1, minWidth: 0, fontSize: '12.5px', color: 'var(--color-gray-500)', fontWeight: 500, textAlign: 'left' }">Add a project…</span>
        <AppIcon name="plus" :size="12" :extra-style="{ color: 'var(--color-gray-500)', flexShrink: 0 }" />
      </button>

      <!-- Populated: project picker -->
      <template v-else>
        <button ref="triggerRef" :style="s.projBtn(projMenuOpen)" @click="projMenuOpen = !projMenuOpen">
          <AppIcon name="folder-git-2" :extra-style="{ color: 'var(--color-amber)', flexShrink: 0 }" :size="13" />
          ...
          <AppIcon :name="projMenuOpen ? 'chevron-down' : 'chevrons-up-down'" :size="13" :extra-style="{ color: 'var(--color-gray-500)', flexShrink: 0 }" />
        </button>
        ...
        <div v-if="projMenuOpen" ref="menuRef" :style="s.projMenu">
          ...
          >
            ...
            >
              <AppIcon :name="p.id === currentProjectId ? 'check' : 'folder'" :size="12" :extra-style="{ color: p.id === currentProjectId ? 'var(--color-amber)' : 'var(--color-gray-500)' }" />
              <div style="flex:1;min-width:0">
                <div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ p.name }}</div>
                <div :style="{ ...s.projMeta, color: 'var(--color-gray-600)' }">{{ p.branch }}</div>
              </div>
            </button>
            <button
          ...
          <button :style="s.projMenuFoot" @click="$emit('add-project'); projMenuOpen = false">
            <AppIcon name="plus" :size="12" :extra-style="{ color: 'var(--color-gray-300)' }" />
            Add new project…
          </button>
        </div>
      </template>
    ...
    <div :style="s.wsList">
      <template v-if="projects.length === 0">
        ...
        <div :style="s.wsEmptyHint">
          <AppIcon name="layers" :size="14" :extra-style="{ color: 'var(--color-gray-700)', flexShrink: 0 }" />
          <span style="font-size:12px;color:var(--color-gray-600);line-height:1.5">Add a project to see workspaces here.</span>
        </div>
      </template>
    ...
    <div :style="s.bottomSection">
      ...
      >
        <AppIcon name="terminal" :size="14" :extra-style="{ color: hoveredDebug ? 'var(--color-amber)' : 'var(--color-gray-300)' }" />
        <span :style="{ fontSize: '13px', color: hoveredDebug ? 'var(--color-fg-primary)' : 'var(--color-gray-100)', fontWeight: 500 }">Hook Debug</span>
      </button>
      <button
      ...
      >
        <AppIcon name="settings" :size="14" :extra-style="{ color: hoveredSettings ? 'var(--color-amber)' : 'var(--color-gray-300)' }" />
        <span :style="{ fontSize: '13px', color: hoveredSettings ? 'var(--color-fg-primary)' : 'var(--color-gray-100)', fontWeight: 500 }">Settings</span>
        <span style="margin-left:auto;font-size:10px;color:var(--color-gray-600);font-family:'JetBrains Mono','Courier Prime',monospace">v0.3</span>
      </button>
    </div>
  </div>
</template>
...
const STATUS_COLORS: Record<string, string> = {
  ...
  grooming: 'var(--color-amber)',
  awaiting_execution: 'var(--color-info)',
...
const s = {
  sidebar: {
    width: '248px', height: '100vh', background: 'var(--color-surface-0)',
  ...
  projBtn: (open: boolean) => ({
    ...
    padding: '9px 11px',
    background: open ? 'var(--color-gray-800)' : 'var(--color-surface-1)',
  ...
  projMeta: {
    ...
    fontSize: '10px', color: 'var(--color-gray-500)', marginTop: '1px', lineHeight: 1.3,
    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
  },
  projMenu: {
  ...
  wsLabel: {
    ...
    padding: '14px 16px 8px', fontFamily: 'var(--font-body)',
    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
  },
  wsCount: { color: 'var(--color-gray-600)', fontWeight: 600, fontSize: '10px' },
  wsList: {
  ...
  wsStatusPip: (status: string) => ({
    ...
    background: STATUS_COLORS[status] || 'var(--color-gray-500)',
    flexShrink: 0,
