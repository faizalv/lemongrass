<template>
  <button :style="itemStyle" @click="$emit('select')">
    <div style="display:flex;align-items:center;gap:8px">
      ...
      <span :style="{ fontSize:'12.5px', fontWeight:600, color: active ? 'var(--color-amber)' : 'var(--color-gray-100)', flex:1, lineHeight:1.4 }">
        {{ task.title }}
      </span>
    </div>
    <div :style="meta">
      ...
      <AppIcon name="check-circle-2" :size="9" :extra-style="{ color: 'var(--color-success)' }" />
...
const itemStyle = {
  ...
  background: props.active ? 'rgba(245,197,24,0.08)' : 'var(--color-gray-900)',
