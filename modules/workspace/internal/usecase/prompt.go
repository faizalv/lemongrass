...
func buildExecutionPrompt(projectAlias, handoverContext string) string {
	...
	body := strings.Join([]string{
		...
		"",
		"#lg.tasks.read -- approved task list with title, reason, impl, task_id. Call this first.",
		lgprompt.CmdTasksStart,
		lgprompt.CmdTasksFinish,
		...
		"--- Task loop ---",
		"",
		"1. #lg.tasks.read -- get approved list with task_id, title, reason, impl",
		"2. For each task in order:",
		"   a. #lg.tasks.start <task_id> -- marks in_progress; check response for rejection notes",
		"   b. Implement the task exactly as described",
		"   c. #lg.tasks.finish <task_id>:<notes> -- marks done; notes: one sentence explaining any divergence from planned impl, or empty if none; check response for rejection notes",
		"3. If a rejection note appears: finish all in-progress work first, then re-do the rejected task before proceeding",
		"4. #lg!.done only when all tasks are done and no rejections are pending",
		"",
		lgprompt.EchoRule,
		"  Starting task 1 -- adding tenant_id filter",
		"  Finished task 1",
		"  Starting task 2 -- creating tenant_middleware.go",
		"",
		"--- Rules ---",
		"",
