...
Lemongrass is another layer of abstraction built on top of Claude Code. The idea was to build an infrastructure for Claude so it can perform fully autonomous work without losing coherence and keep the code quality consistent after several runs. It runs on your Claude subscription, not an API key. We run Claude Code inside a container via PTY, it basically sees Lemongrass as a proper terminal it talks to.
