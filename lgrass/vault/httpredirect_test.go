...
func newRedirectFixture(t *testing.T, placement TokenPlacement, redirectTo func(other *httptest.Server) string) (*Service, ChannelID, *[]seenRequest) {
	t.Helper()
	...
	domain := Domain{
		...
		Users:           []DomainUser{{Name: "alice", Fields: map[string]string{"u": "alice"}}},
	}
	if err := svc.PutDomain("root-secret", "d", domain); err != nil {
		t.Fatal(err)
	}
	c, err := svc.CreateHTTPChannel("root-secret", "ch", "d", HTTPScope{Methods: []string{"GET"}}, time.Hour)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCrossHostRedirectDropsTheToken(t *testing.T) {
	placements := map[string]TokenPlacement{
		"authorization header": {Kind: PlacementHeader, Name: "Authorization", Prefix: "Bearer "},
		"custom header":        {Kind: PlacementHeader, Name: "X-Api-Key"},
		"cookie":               {Kind: PlacementCookie, Name: "session"},
		"query":                {Kind: PlacementQuery, Name: "access_token"},
	}
...
func TestLoginRedirectToAnotherHostIsRefused(t *testing.T) {
	...
	domain := Domain{BaseURL: base.URL, LoginEndpoint: "/login", TokenPath: "access_token", FixedTTLSeconds: 60}
	user := DomainUser{Name: "alice", Fields: map[string]string{"password": "s3cret-pw"}}
