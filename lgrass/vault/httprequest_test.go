...
func setUpHTTPChannel(t *testing.T, srv *httptest.Server, users []DomainUser, methods []string) (*Service, ChannelID) {
	t.Helper()
	...
	c, err := svc.CreateHTTPChannel(rootSecret, "test-channel", "test-domain", HTTPScope{Methods: methods}, time.Hour)
	if err != nil {
...
func TestRequestHTTPExpiredChannelDenied(t *testing.T) {
	...
	c, err := svc.loadHTTPMeta(id)
	if err != nil {
		t.Fatalf("loadHTTPMeta: %v", err)
	}
	c.ExpiresAt = time.Now().Add(-time.Minute)
	if err := svc.saveHTTPMeta(c); err != nil {
