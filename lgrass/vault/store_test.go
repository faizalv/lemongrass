...
func TestStorePutGetRoundTrip(t *testing.T) {
	...
	if err := store.Put("kencana-backend", key, want); err != nil {
		t.Fatalf("Put: %v", err)
	}
	got, err := store.Get("kencana-backend", key)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
...
func TestStoreGetWrongKeyFails(t *testing.T) {
	store, key := openTestStore(t)
	if err := store.Put("kencana-backend", key, []byte("secret")); err != nil {
		t.Fatalf("Put: %v", err)
	}
	...
	if _, err := store.Get("kencana-backend", wrongKey); err == nil {
...
func TestStoreGetMissingReturnsErrNotFound(t *testing.T) {
	store, key := openTestStore(t)
	if _, err := store.Get("hr-backend", key); !errors.Is(err, ErrNotFound) {
...
func TestStoreDeleteThenGetReturnsErrNotFound(t *testing.T) {
	...
	if err := store.Delete("kencana-backend"); err != nil {
		t.Fatalf("Delete: %v", err)
	}
	if _, err := store.Get("kencana-backend", key); !errors.Is(err, ErrNotFound) {
